...
initialize_owaiz_vm() {
    ...
    if ! sudo lxc profile show allow-internet &>/dev/null; then
        ...
        # Assume lxdbr0 is your bridge network and 10.1.1.1/24 is your subnet
        sudo lxc profile device add allow-internet eth0 nic nictype=bridged parent=lxdbr0 name=eth0
    fi

    local configs=(
        #"Tiny,4,4GiB,10GiB,10.1.1.101"
        "Small2,4,4GiB,40GiB,10.1.1.110"
	"Small3,4,4GiB,40GiB,10.1.1.111"
	"Small4,4,4GiB,40GiB,10.1.1.122"
	"Small5,4,4GiB,40GiB,10.1.1.123"
	"Small6,4,4GiB,40GiB,10.1.1.1242"
